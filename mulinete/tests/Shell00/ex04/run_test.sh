...
# separados por ", ", directorys com "/" no final, sem arquivos ocultos
...
if [ ! -f "$MIDLS" ]; then
    echo "[ex04] FAIL: arquivo 'midLS' not found em ex04/"
    exit $FAIL
fi
...
# Executa o midLS no directory de teste
...
# Teste 3: directorys terminam com /
if ! echo "$OUTPUT" | grep -q "subdir1/"; then
    echo "[ex04] FAIL: directorys deveriam terminar com '/'"
    rm -rf "$TMPDIR"
    exit $FAIL
fi
