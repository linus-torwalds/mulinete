#!/bin/sh
# ex05: git_commit.sh deve exibir exatamente os ultimos 5 commits (SHA completo, um por line)
...
if [ ! -f "$SCRIPT" ]; then
    echo "[ex05] FAIL: arquivo 'git_commit.sh' not found em ex05/"
    exit $FAIL
fi
...
# Teste 3: exatamente 5 lines
linecount=$(echo "$OUTPUT" | wc -l | tr -d ' ')
if [ "$linecount" != "5" ]; then
    echo "[ex05] FAIL: deveria exibir 5 lines, exibiu $linecount"
    exit $FAIL
fi

# Teste 4: cada line e um SHA valido (40 caracteres hexadecimais)
ERRORS=0
line_num=0
echo "$OUTPUT" | while IFS= read -r line; do
    line_num=$((line_num + 1))
    if ! echo "$line" | grep -qE '^[0-9a-f]{40}$'; then
        echo "[ex05] FAIL: line $line_num nao e um SHA valido: '$line'"
        ERRORS=$((ERRORS + 1))
    fi
done
