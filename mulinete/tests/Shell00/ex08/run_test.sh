#!/bin/sh
# ex08: 'clean' deve ser um unico comando find que:
...
CLEANFILE="../ex08/clean"

# Teste 1: arquivo existe
if [ ! -f "$CLEANFILE" ]; then
    echo "[ex08] FAIL: arquivo 'clean' not found em ex08/"
    exit $FAIL
fi
...
# Remove comentarios e lines vazias para contar comandos reais
cmd_lines=$(echo "$content" | grep -v '^\s*#' | grep -v '^\s*$')
if echo "$cmd_lines" | grep -qE '(;|&&|\|\|)'; then
    echo "[ex08] FAIL: nao pode usar ';', '&&' ou '||'"
    exit $FAIL
fi
...
# Executa o clean no directory de teste
...
for f in "file~" "backup~" "#draft#" "#notes#" "subdir/nested~" "subdir/#temp#"; do
    if [ -e "$TMPDIR/$f" ]; then
        echo "[ex08] FAIL: '$f' should have sido deletado mas ainda existe"
        ERRORS=$((ERRORS + 1))
    fi
done
...
for f in "normal_file" "file.c" "#incomplete" "incomplete#" "subdir/keep_me"; do
    if [ ! -e "$TMPDIR/$f" ]; then
        echo "[ex08] FAIL: '$f' nao should have sido deletado"
        ERRORS=$((ERRORS + 1))
    fi
done
