...
check_perms() {
    ...
    if [ "$perms" != "$expected" ]; then
        echo "[ex00] FAIL: $name - incorrect permissions. Expected $expected, got $perms"
        ERRORS=$((ERRORS + 1))
    fi
}
...
check_type() {
    ...
    if [ "$type" = "f" ] && [ ! -f "$file" ]; then
        echo "[ex00] FAIL: $name - should be a regular file"
        ERRORS=$((ERRORS + 1))
    elif [ "$type" = "d" ] && [ ! -d "$file" ]; then
        echo "[ex00] FAIL: $name - should be a directory"
        ERRORS=$((ERRORS + 1))
    elif [ "$type" = "l" ] && [ ! -L "$file" ]; then
        echo "[ex00] FAIL: $name - should be a symbolic link"
        ERRORS=$((ERRORS + 1))
    fi
}
...
# Teste 1: tar existe
if [ ! -f "$TARFILE" ]; then
    echo "[ex00] FAIL: arquivo 'exo2.tar' not found em ex00/"
    rm -rf "$TMPDIR"
    exit $FAIL
fi
...
if ! tar -xf "$TARFILE" -C "$TMPDIR" 2>/dev/null; then
    echo "[ex00] FAIL: could not extract o tar"
    rm -rf "$TMPDIR"
    exit $FAIL
fi
...
if [ "$inode3" != "$inode5" ]; then
    echo "[ex00] FAIL: test3 e test5 should be hard links (mesmo inode)"
    ERRORS=$((ERRORS + 1))
fi
...
if [ "$link_target" != "test0" ]; then
    echo "[ex00] FAIL: test6 should point to 'test0', aponta para '$link_target'"
    ERRORS=$((ERRORS + 1))
fi
...
if [ "$size1" != "4" ]; then
    echo "[ex00] FAIL: test1 should have 4 bytes, got $size1"
    ERRORS=$((ERRORS + 1))
fi

size3=$(wc -c < "$TMPDIR/test3" 2>/dev/null | tr -d ' ')
if [ "$size3" != "1" ]; then
    echo "[ex00] FAIL: test3 should have 1 byte, tem $size3"
    ERRORS=$((ERRORS + 1))
fi

size4=$(wc -c < "$TMPDIR/test4" 2>/dev/null | tr -d ' ')
if [ "$size4" != "2" ]; then
    echo "[ex00] FAIL: test4 should have 2 bytes, got $size4"
    ERRORS=$((ERRORS + 1))
fi
