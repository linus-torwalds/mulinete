...
if [ ! -f "$ARQUIVO_ALUNO" ]; then
    echo "[ex03] FAIL: Arquivo 'find_sh.sh' not found em ex03/."
    exit $FAIL
fi
...
for name in root_script deep_script very_deep hidden_script; do
    if ! echo "$OUTPUT" | grep -q "^$name$"; then
        echo "[ex03] FAIL: '$name' should be in the output, mas não foi encontrado."
        ERRORS=$((ERRORS + 1))
    fi
done
...
if echo "$OUTPUT" | grep -q '\.sh'; then
    echo "[ex03] FAIL: A extensão '.sh' was detected in the output. Should display only the name."
    ERRORS=$((ERRORS + 1))
fi

# Teste C: Proibição de caminhos (paths) [cite: 329, 331]
if echo "$OUTPUT" | grep -q '/'; then
    echo "[ex03] FAIL: Caminhos de directory detectados. Display only the filename."
    ERRORS=$((ERRORS + 1))
fi
...
for intruder in not_sh wrong_ext .sh sh; do
    if echo "$OUTPUT" | grep -q "^$intruder$"; then
        echo "[ex03] FAIL: O arquivo '$intruder' was listed incorrectly."
        ERRORS=$((ERRORS + 1))
    fi
done
