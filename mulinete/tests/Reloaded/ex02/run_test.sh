...
if [ ! -f "$CLEANFILE" ]; then
    echo "[ex02] FAIL: arquivo 'clean' not found em ex02/"
    exit $FAIL
fi
...
for f in "file~" "backup~" "#draft#" "#notes#" "subdir/nested~" "subdir/#temp#"; do
    if [ -e "$TMPDIR/$f" ]; then
        echo "[ex02] FAIL: '$f' should have sido deletado mas ainda existe"
        ERRORS=$((ERRORS + 1))
    fi
done
...
for f in "normal_file" "file.c" "#incomplete" "incomplete#" "subdir/keep_me"; do
    if [ ! -e "$TMPDIR/$f" ]; then
        echo "[ex02] FAIL: '$f' nao should have sido deletado"
        ERRORS=$((ERRORS + 1))
    fi
done
