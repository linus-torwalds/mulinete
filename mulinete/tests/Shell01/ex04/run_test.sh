#!/bin/sh
# ex04: MAC.sh deve exibir enderecos MAC da maquina, um por line
...
if [ ! -f "$SCRIPT" ]; then
    echo "[ex04] FAIL: 'MAC.sh' not found em ex04/"
    exit $FAIL
fi
...
# Teste 3: cada line e um MAC valido
while IFS= read -r line; do
    [ -z "$line" ] && continue
    if ! echo "$line" | grep -qiE '^([0-9a-f]{2}[:\-]){5}[0-9a-f]{2}$'; then
        echo "[ex04] FAIL: line nao e um MAC valido: '$line'"
        ERRORS=$((ERRORS + 1))
    fi
done << EOF
$OUTPUT
