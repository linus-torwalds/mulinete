#!/bin/sh
# ex03: count_files.sh deve contar arquivos regulares e diretorios (incluindo ".")
...
if [ ! -f "$SCRIPT" ]; then
    echo "[ex03] FAIL: 'count_files.sh' nao encontrado em ex03/"
    exit $FAIL
fi
...
if ! echo "$OUTPUT" | grep -qE '^[0-9]+$'; then
    echo "[ex03] FAIL: saida deve ser um numero, obtido: '$OUTPUT'"
    rm -rf "$TMPDIR"
    exit $FAIL
fi
...
if [ "$OUTPUT" != "$EXPECTED" ]; then
    echo "[ex03] FAIL: contagem incorreta. Esperado $EXPECTED, obtido $OUTPUT"
    rm -rf "$TMPDIR"
    exit $FAIL
fi

# Teste 4: saida e uma unica linha
linecount=$(echo "$OUTPUT" | wc -l | tr -d ' ')
if [ "$linecount" != "1" ]; then
    echo "[ex03] FAIL: saida deve ter 1 linha, obtido $linecount"
    rm -rf "$TMPDIR"
    exit $FAIL
fi
...
if [ "$OUTPUT2" != "$EXPECTED2" ]; then
    echo "[ex03] FAIL: contagem incorreta apos adicionar itens. Esperado $EXPECTED2, obtido $OUTPUT2"
    rm -rf "$TMPDIR"
    exit $FAIL
fi
