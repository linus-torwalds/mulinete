#!/bin/sh
# ex06: skip.sh deve exibir apenas linhas impares do ls -l (linha 1, 3, 5...)
...
if [ ! -f "$SCRIPT" ]; then
    echo "[ex06] FAIL: 'skip.sh' nao encontrado em ex06/"
    exit $FAIL
fi
...
# Captura ls -l real do diretorio
...
# Teste 3: constroi o esperado (linhas 1, 3, 5... do ls -l)
...
# Teste 4: numero de linhas deve ser ceil(total/2)
...
if [ "$OUTPUT_LINES" != "$EXPECTED_LINES" ]; then
    echo "[ex06] FAIL: numero de linhas incorreto. Esperado $EXPECTED_LINES, obtido $OUTPUT_LINES"
    ERRORS=$((ERRORS + 1))
fi
