...
if [ ! -f "$CLEANFILE" ]; then
    echo "[ex08] FAIL: arquivo 'clean' nao encontrado em ex08/"
    exit $FAIL
fi
...
for f in "file~" "backup~" "#draft#" "#notes#" "subdir/nested~" "subdir/#temp#"; do
    if [ -e "$TMPDIR/$f" ]; then
        echo "[ex08] FAIL: '$f' deveria ter sido deletado mas ainda existe"
        ERRORS=$((ERRORS + 1))
    fi
done
...
for f in "normal_file" "file.c" "#incomplete" "incomplete#" "subdir/keep_me"; do
    if [ ! -e "$TMPDIR/$f" ]; then
        echo "[ex08] FAIL: '$f' nao deveria ter sido deletado"
        ERRORS=$((ERRORS + 1))
    fi
done
