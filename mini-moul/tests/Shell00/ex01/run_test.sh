...
if [ ! -f "$TARFILE" ]; then
    echo "[ex01] FAIL: arquivo 'testShell00.tar' nao encontrado em ex01/"
    rm -rf "$TMPDIR"
    exit $FAIL
fi
...
if ! tar -xf "$TARFILE" -C "$TMPDIR" 2>/dev/null; then
    echo "[ex01] FAIL: nao foi possivel extrair o tar"
    rm -rf "$TMPDIR"
    exit $FAIL
fi

if [ ! -f "$TMPDIR/testShell00" ]; then
    echo "[ex01] FAIL: arquivo 'testShell00' nao encontrado dentro do tar"
    rm -rf "$TMPDIR"
    exit $FAIL
fi
...
if [ "$perms" != "455" ]; then
    echo "[ex01] FAIL: permissoes incorretas. Esperado 455, obtido $perms"
    rm -rf "$TMPDIR"
    exit $FAIL
fi
...
if [ "$filecount" != "1" ]; then
    echo "[ex01] FAIL: tar contem $filecount arquivo(s), esperado apenas 1 (testShell00)"
    rm -rf "$TMPDIR"
    exit $FAIL
fi
