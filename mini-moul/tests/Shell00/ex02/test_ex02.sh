...
check_perms() {
    ...
    if [ "$perms" != "$expected" ]; then
        echo "[ex02] FAIL: $name - permissoes incorretas. Esperado $expected, obtido $perms"
        ERRORS=$((ERRORS + 1))
    fi
}
...
check_type() {
    ...
    if [ "$type" = "f" ] && [ ! -f "$file" ]; then
        echo "[ex02] FAIL: $name - deveria ser um arquivo regular"
        ERRORS=$((ERRORS + 1))
    elif [ "$type" = "d" ] && [ ! -d "$file" ]; then
        echo "[ex02] FAIL: $name - deveria ser um diretorio"
        ERRORS=$((ERRORS + 1))
    elif [ "$type" = "l" ] && [ ! -L "$file" ]; then
        echo "[ex02] FAIL: $name - deveria ser um link simbolico"
        ERRORS=$((ERRORS + 1))
    fi
}
...
# Teste 1: tar existe
if [ ! -f "$TARFILE" ]; then
    echo "[ex02] FAIL: arquivo 'exo2.tar' nao encontrado em ex02/"
    rm -rf "$TMPDIR"
    exit $FAIL
fi
...
if ! tar -xf "$TARFILE" -C "$TMPDIR" 2>/dev/null; then
    echo "[ex02] FAIL: nao foi possivel extrair o tar"
    rm -rf "$TMPDIR"
    exit $FAIL
fi
...
if [ "$inode3" != "$inode5" ]; then
    echo "[ex02] FAIL: test3 e test5 deveriam ser hard links (mesmo inode)"
    ERRORS=$((ERRORS + 1))
fi
...
if [ "$link_target" != "test0" ]; then
    echo "[ex02] FAIL: test6 deveria apontar para 'test0', aponta para '$link_target'"
    ERRORS=$((ERRORS + 1))
fi
...
if [ "$size1" != "4" ]; then
    echo "[ex02] FAIL: test1 deveria ter 4 bytes, tem $size1"
    ERRORS=$((ERRORS + 1))
fi

size3=$(wc -c < "$TMPDIR/test3" 2>/dev/null | tr -d ' ')
if [ "$size3" != "1" ]; then
    echo "[ex02] FAIL: test3 deveria ter 1 byte, tem $size3"
    ERRORS=$((ERRORS + 1))
fi

size4=$(wc -c < "$TMPDIR/test4" 2>/dev/null | tr -d ' ')
if [ "$size4" != "2" ]; then
    echo "[ex02] FAIL: test4 deveria ter 2 bytes, tem $size4"
    ERRORS=$((ERRORS + 1))
fi
