#!/bin/sh
# ex02: find_sh.sh deve listar todos os .sh no dir atual e subdiretórios, sem extensao
...
if [ ! -f "$SCRIPT" ]; then
    echo "[ex02] FAIL: 'find_sh.sh' nao encontrado em ex02/"
    exit $FAIL
fi
...
for name in file1 file2 file3 file4; do
    if ! echo "$OUTPUT" | grep -q "^$name$"; then
        echo "[ex02] FAIL: '$name' nao encontrado na saida (sem extensao .sh)"
        ERRORS=$((ERRORS + 1))
    fi
done
...
# Teste 5: cada entrada e uma unica linha (sem paths, so o nome)
