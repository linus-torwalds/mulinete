...
if [ ! -f "$FILEPATH" ]; then
    echo "[ex05] FAIL: arquivo '$TARGET_NAME' nao encontrado em ex05/"
    exit $FAIL
fi
...
if [ "$content" != "42" ]; then
    echo "[ex05] FAIL: conteudo incorreto. Esperado '42', obtido '$content'"
    exit $FAIL
fi
...
if [ "$size" != "3" ]; then
    echo "[ex05] FAIL: tamanho incorreto. Esperado 3 bytes, obtido $size"
    exit $FAIL
fi
