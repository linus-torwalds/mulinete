...
if [ ! -f "../ex01/z" ]; then
    echo "[ex01] FAIL: arquivo 'z' nao encontrado em ex01/"
    exit $FAIL
fi
...
content=$(cat "../ex01/z")
if [ "$content" != "Z" ]; then
    echo "[ex01] FAIL: conteudo incorreto. Esperado 'Z', obtido '$content'"
    exit $FAIL
fi
...
size=$(echo "$size" | tr -d ' ')
if [ "$size" != "2" ]; then
    echo "[ex01] FAIL: tamanho incorreto. Esperado 2 bytes, obtido $size bytes"
    exit $FAIL
fi
