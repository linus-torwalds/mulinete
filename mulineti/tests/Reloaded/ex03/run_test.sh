...
for name in root_script deep_script very_deep hidden_script; do
    if ! echo "$OUTPUT" | grep -q "^$name$"; then
        echo "[ex03] FAIL: '$name' deveria estar na saída, mas não foi encontrado."
        ERRORS=$((ERRORS + 1))
    fi
done
...
if echo "$OUTPUT" | grep -q '\.sh'; then
    echo "[ex03] FAIL: A extensão '.sh' foi detectada na saída. Deve exibir apenas o nome."
    ERRORS=$((ERRORS + 1))
fi

# Teste C: Proibição de caminhos (paths) [cite: 329, 331]
if echo "$OUTPUT" | grep -q '/'; then
    echo "[ex03] FAIL: Caminhos de diretório detectados. Exiba apenas o nome do arquivo."
    ERRORS=$((ERRORS + 1))
fi
...
for intruder in not_sh wrong_ext .sh sh; do
    if echo "$OUTPUT" | grep -q "^$intruder$"; then
        echo "[ex03] FAIL: O arquivo '$intruder' foi listado incorretamente."
        ERRORS=$((ERRORS + 1))
    fi
done
